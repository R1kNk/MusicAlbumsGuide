...
namespace MusicAlbumsGuide.UserControls
{
    public partial class EditAlbumForm : Form
    {
        ...
        int AlbumId;
        object Sender;
        public EditAlbumForm(Form1 parentForm1, int albumId, object sender)
        {
            Opacity = 0;
        ...
        {
            parentForm.Enabled = true;
            if (Sender is BigAlbumView)
            {
                parentForm.ShowBigViewAlbum(SharedContextMethods.GetFullAlbum(AlbumId));
                Close();
            }
            else
            {
        ...
        {
             image = new ImageForm(this, CoverAlbum.Image);
            image.Show();
        }

        private void EditAlbumForm_Activated(object sender, EventArgs e)
        {
            if (image != null)
            {
                if (!image.IsDisposed)
        ...
        {
            MusicAlbumsGuideDB context = new MusicAlbumsGuideDB();
            Album newAlbum = context.Albums.Where(p => p.AlbumId == AlbumId).Single();
